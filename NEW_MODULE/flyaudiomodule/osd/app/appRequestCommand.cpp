...
void setOsdBar(void)
{
   char buff[]={0x02,0x28,0x00}; // jing 2013-8-2 send 0x02 0x28 0x00 to UI to show mp3 osd
   setOsdInfoControl((u8 *)buff,3);
}
...
void sendOsdDebugInfo(u8 *param, u8 len){
	...
	buffer[0] = 0xFF;
	memcpy(&buffer[1], param, len);
	setCommand(0x00000000, buffer, len+1);
}

// jing 2013-8-12 low better osd {
void sendLowBetterOSD(void){
	u8 buffer[] = {0x02,0x40,0x01};
	u8 len  = 3;
	setCommand(0x00000000, buffer, len);
}
// jing 2013-8-12 low better osd }
