...
void setAnalogData(u32 ctrlId, int analog){
	...
	LOGD("ctrlId=0x%x,analog=%d",ctrlId,analog);
...
{
	u8 buf[] = {0xA9,0x01,param};
	setCommand(0x00000000, buf, sizeof(buf));
}

void extcarToApp_OpenBombBox_RadarData(u8 pos,u8 *param)
{
	u8 buf[] = {0xA9,0x02,0xFF,0xFF,0xFF,0xFF,0xFF};
	buf[2] = pos;
	memcpy(&buf[3],param,4);
	setCommand(0x00000000, buf, sizeof(buf));
}

void extcarToApp_SetSystemLanguage(u8 param)
{
	u8 buf[] = {0xa5,0x03,param};
	setCommand(0x00000000, buf, sizeof(buf));
}
