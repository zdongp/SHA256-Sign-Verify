...
{
	setCommand(0x00000000,param,paramLen);
}
...
void clearInputFreq(void)
{
	...
	memset(pLocalParam->inputFrequency.frequencyNumber,0,INPUT_FREQUENCY_MAX_LEN);
	pLocalParam->inputFrequency.len = 0;
}
void changeMinFreqPage()
{
	u8 nChanel = pLocalParam->eRadioChannel;
	pLocalParam->stszRadioChannelRecord[nChanel ].curPage--;
	if(pLocalParam->stszRadioChannelRecord[nChanel ].curPage < 1)
	{
		pLocalParam->stszRadioChannelRecord[nChanel ].curPage = pLocalParam->stszRadioChannelRecord[nChanel ].totalPages;
	}
}
void changeMaxFreqPage(void)
{
	u8 nChanel = pLocalParam->eRadioChannel;
	pLocalParam->stszRadioChannelRecord[nChanel ].curPage++;
	if(pLocalParam->stszRadioChannelRecord[nChanel ].curPage > pLocalParam->stszRadioChannelRecord[nChanel ].totalPages)
	{
		pLocalParam->stszRadioChannelRecord[nChanel ].curPage = 1;
	}
	/*if(pLocalParam->stszRadioChannelRecord[nChanel ].curPage >= FREQUENCY_MAX_PAGE)
	{
		pLocalParam->stszRadioChannelRecord[nChanel ].curPage = FREQUENCY_MAX_PAGE;
	}*/
}

/*bool isSpacePage(int page)
{
 ...
 {
 	u8 nChanel = pLocalParam->eRadioChannel;
	short totalPages = pLocalParam->stszRadioChannelRecord[nChanel].totalPages;
	int baseIndex = (totalPages - 1) * CHANNEL_RECORD_NUM;
	int i = 0;
	for(; i < CHANNEL_RECORD_NUM; i++)
	{
		if( pLocalParam->stszRadioChannelRecord[nChanel].bRadioFreqRecordState[baseIndex + i])
			break;
	}
	if(CHANNEL_RECORD_NUM == i)//current page is a space page
	{
		if(totalPages == 1)//if only one page,  return
			return;

		int j = 0;
		
		baseIndex = baseIndex - CHANNEL_RECORD_NUM;
		for(; j < CHANNEL_RECORD_NUM; j++)
		{
			if(pLocalParam->stszRadioChannelRecord[nChanel].bRadioFreqRecordState[baseIndex + j])
			{
				break;
			}

		}
		if(j == CHANNEL_RECORD_NUM)//page-1 is a space
		{

			pLocalParam->stszRadioChannelRecord[nChanel].totalPages --;
		}

	}
	else//not space
	{
		if(pLocalParam->stszRadioChannelRecord[nChanel].totalPages == FREQUENCY_MAX_PAGE)
			return ;
		pLocalParam->stszRadioChannelRecord[nChanel].totalPages ++;
	}
	RadioToApp_UpdatePage(nChanel);
 }
