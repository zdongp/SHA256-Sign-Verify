...
void CBtJniSdk::analysSdkToBtStatusMessage(u8 *p,u8 len)
{
	switch(p[0])
	{
		...
		case 0x04:
			{
				sendToAppBtSongInfo(m_pBtParam->musicName,m_pBtParam->musicAuthor,m_pBtParam->musicAlbum,
					strlen(m_pBtParam->musicName),strlen(m_pBtParam->musicAuthor),strlen(m_pBtParam->musicAlbum));
			}
			break;
		case 0x05:
			{
				sendToAppConnectBtName(m_pBtParam->szRemodeBtDeviceHasConnectedName,strlen(m_pBtParam->szRemodeBtDeviceHasConnectedName));
			}
			break;
		case 0x06:
			{
				sendToAppMusicPlayStatus(m_pBtParam->m_bIsA2dpOnPlayState);
			}
			break;
		case 0x07:
             sendToAppReceiveSound(m_pBtParam->m_eHfpAudioSource);
            break;
		case 0x08:
             sendToAppMicStatus(m_pBtParam->m_bIsMicOpen);
            break;
		  default:break;
	}
}
...
void CBtJniSdk::analyseSdkToBtSetSongPlay(u8 param)
{
	switch(param)
	{
		case 0x00:
...
/*****************************SendToSDKFunction************************************/
 void CBtJniSdk::sendToFlyJniSdkService(u8 moduleId, u8 *p, u8 len)
 {
 ...
 void CBtJniSdk::sendToAppBtSongInfo(char *song,char *auther,char *album,u8 songLen,u8 autherLen,u8 albumLen)
 {
	u8 szMsg[256*3];
	memset(szMsg,0,sizeof(szMsg));
	szMsg[0] = CMDTYPESTATUS;
	szMsg[1] = BT_A2DP_SONG_INFO;
	memcpy(szMsg + 2,song,songLen);
	int len1= songLen + 2;
	
	szMsg[len1] = 0xff;
	memcpy(szMsg +len1+1,auther,autherLen);
	int len2= len1 +1 + autherLen;
	
	szMsg[len2] = 0xff;
	memcpy(szMsg + len2 +1,album,albumLen);
	int len3 = len2 +1+ albumLen;
	
	sendToFlyJniSdkService(MODULE_SDK_BT, szMsg,len3);
 }

 void CBtJniSdk::sendToAppConnectBtName(char *name,int len)
 {
	u8 szMsg[256*3];
	memset(szMsg,0,sizeof(szMsg));
	szMsg[0] = CMDTYPESTATUS;
	szMsg[1] = 0x04;
	memcpy(szMsg + 2,name,len);
	sendToFlyJniSdkService(MODULE_SDK_BT, szMsg,len + 2);
 }

 void CBtJniSdk::sendToAppMusicPlayStatus(bool cState)
 {
	u8 szMsg[3] = {CMDTYPESTATUS,0x05,cState};
	sendToFlyJniSdkService(MODULE_SDK_BT, szMsg, 3);
 }
void CBtJniSdk::sendToAppReceiveSound(u8 param)
{
       u8 szMsg[3] = {CMDTYPESTATUS,0x06,param};
       sendToFlyJniSdkService(MODULE_SDK_BT, szMsg, 3);
}
void CBtJniSdk::sendToAppMicStatus(u8 param)
{
       u8 szMsg[3] = {CMDTYPESTATUS,0x07,param};
       sendToFlyJniSdkService(MODULE_SDK_BT, szMsg, 3);
}
