...
int TimerHandlerCenter(timer_t timerId){
	LOGI("AUX timer handler");

		switch((s64)timerId)
		{
			case AUX_OSD_TIMER_HIDDEN_DISPLAY:
				{
					//
					//		u16 curPage = 0;
					//		u8 curPageLen = 0;
					//		getOtherModuleParam(MODULE_ID_SYSTEM,CUR_PAGE, (u8 *)&curPage,&curPageLen);
					//		if(curPage == PAGE_AUX_VIDEO_CONTROL || curPage == PAGE_AUX_VIDEO_SET){
...
void initTimerCenter(timer_handler_function func){
    ...
    makeAndSendMessageToModule(MODULE_ID_AUX,MODULE_ID_TIMER,buf,5);
}
...
int SetTimer(int nElaspe_ms, int nMode, int timerId){
    ...
    makeAndSendMessageToModule(MODULE_ID_AUX,MODULE_ID_TIMER,buf,10);
