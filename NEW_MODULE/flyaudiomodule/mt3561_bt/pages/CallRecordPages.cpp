	...
	case MOUSE_STATUS_TIMER:
		{
            ...
            if(BT_PBAP_SYNC_PATH_RECEIVE_CALLHISTORY == m_pBtParam->m_iselectPhoneBook)
				refreshHistoryCall_Riceived(m_nCallLogBrowseFrom);
			else if(BT_PBAP_SYNC_PATH_DIALED_CALLHISTORY == m_pBtParam->m_iselectPhoneBook)
				refreshHistoryCall_Dialed(m_nCallLogBrowseFrom);
			else if(BT_PBAP_SYNC_PATH_MISSED_CALLHISTORY == m_pBtParam->m_iselectPhoneBook)
				refreshHistoryCall_Miss(m_nCallLogBrowseFrom);
			else if(BT_PBAP_SYNC_PATH_ALL_CALLHISTORY == m_pBtParam->m_iselectPhoneBook)
	...
	case MOUSE_STATUS_UP:
		{
			...
			if(BT_PBAP_SYNC_PATH_ALL_CALLHISTORY==m_pBtParam->m_iselectPhoneBook)
			{
			...
			else if(BT_PBAP_SYNC_PATH_RECEIVE_CALLHISTORY == m_pBtParam->m_iselectPhoneBook)
			{
...
void CCallRecordPages::control_CallLogs(int param,E_MOUSE_STATUS e)
{
	...
	switch(param)
	{
		...
		break;
		case 1:
			m_pBtParam->m_emSelectedPhone=PHONEBOOK_DIALED;
		...
		break;
		case 2:
			m_pBtParam->m_emSelectedPhone=PHONEBOOK_MISSED;
...
void CCallRecordPages::getAndShowCallRecords(EM_SYNC_PHONEBOOK_PATH RecordType)
{
	...
	m_pBtParam->m_iselectPhoneBook = RecordType;
...
void CCallRecordPages::createListHistoryCall(void)
{
	...
	switch(m_pBtParam->m_emSelectedPhone)
	{
		case PHONEBOOK_RECV:
		...
		break;
		case PHONEBOOK_DIALED:
			
			getAndShowCallRecords(BT_PBAP_SYNC_PATH_DIALED_CALLHISTORY);
			getAndShowCallRecords_New(BT_PBAP_SYNC_PATH_DIALED_CALLHISTORY);
			
		break;
		case PHONEBOOK_MISSED:
...
void CCallRecordPages::setDataToUI_bt_aboutCallLogCallTime_Displaying(int nPos,char *buff)
{
    ...
    memcpy(&cHour,buff+8,2);
    memcpy(&cMin,buff+10,2);
...
void CCallRecordPages::showLoadingCallLogTips(u32 strTips)
{
    if(strTips != 0)
    {
		...
		printf("\n[showLoadingCallLogTips]    SHOW");
	}
	else
	{
	    setSerialData(CONTROLID_BLUETOOTH_AUDIOVOX_MOBILE_RECORD_LOADING, (u8*)"", 0);
		printf("\n[showLoadingCallLogTips]    HID");
	}
}
void CCallRecordPages::showListHistoryCall(void)
{
	list<ST_CALL_RECORD_ONE_USER_INFO>::iterator it;
	if(m_pBtParam->FirOrBackTag == true)
		{
			it = m_pBtParam->m_listHistoryCall.begin();
		}
	else if(m_pBtParam->FirOrBackTag == false)
		{
			it = (--m_pBtParam->m_listHistoryCall.end());
		}
