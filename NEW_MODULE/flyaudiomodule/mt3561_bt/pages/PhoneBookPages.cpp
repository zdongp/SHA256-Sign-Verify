...
bool CPhoneBookPages::phoneBookSearchPage(u32 id, E_MOUSE_STATUS e, u8 *p, u8 len)
{
	...
	case CONTROLID_BLUETOOTH_AUDIOVOX_CONTACTS_SEARCH_TWO_PAGE_DOWN:
		...
		break;
	//����
	case CONTROLID_BLUETOOTH_AUDIOVOX_CONTACTS_DELETE://�˸��
...
void CPhoneBookPages::getAllPhoneBookLoadDone(void)
{
	...
	printf("getAllPhoneBookLoadDone\n");
	beginSearchKeyWord();
}
...
void CPhoneBookPages::beginSearchKeyWord()
{
	...
	for (map<int, ST_PhoneBookUserInfo* >::iterator it = itFrom; it != itTo; it++)
	{
		...
		else
		{
			...
			if (nLenUtf8 != 0 && nLenAnsi == 0)//˵��û��Ӣ����ĸ
			{
				r = -1;
			}
			else
...
int CPhoneBookPages::findSubStringInUnicodePhoneBook(u8 *pShortString, int nShortStringLen, u8 *pLongString, int nLongStringLen, int &nMatchWeight)
{
	...
	for (i = 0; i < dwUnicodeBufLen; i++)
	{
		...
		else if (oneWord >= FIRST_HANZI_UNICODE && oneWord <= LAST_HANZI_UNICODE)//����
		{
			...
			for (m = 0; m < nLenHanZiPinyin; m++)
			{
				...
				if (m == nLenHanZiPinyin - 1 && n > 0)//������һ��ƴ��
				{
					bOneWordHave = true;
				}
				...
				if (bOneWordHave)
				{
					...
					for (k = 0; k < n; k++)
					{
						if (k == 0 && pShortString[j] == szOnePinyin[k])//ƴ������ĸ 
						{
							j++;
							bMatch = true;
							continue;
			...
			if (nMatchLevel == 1)//ֻ������ĸƥ��
				j = t + 1;

			if (nMatchLevel == 1 || nMatchLevel == 2)//����ĸƥ�����ȫƥ��
			{
				nMatchWeight += getHanZiMatchWeight(i);
			}
...
void CPhoneBookPages::clearPhoneBook(void)
{
	...
	if (!m_mapPhoneNumUserInfo.empty())
	{
		...
		m_mapPhoneNumUserInfo.erase(itFrom, itTo);	//ʹ��ǰ���
	}
}
	...
	case MOUSE_STATUS_UP:
		{
			if (it != itTo)
			{
				...
				m_nSearchPageActiveIndex  = i;

				refreshSearchResultPageSearchResult(i);

				setDigitalData(CONTROLID_BLUETOOTH_AUDIOVOX_CONTACTS_TEXT_1, 1);
				...
				{
					m_nSearchResultPageActiveIndex = m_nSearchPageActiveIndex;
...
wchar_t CPhoneBookPages::getPhoneBookKeyBoardKeyNameById(E_MOUSE_STATUS e, int id)
{
	...
	if (id >= CONTROLID_BLUETOOTH_AUDIOVOX_CONTACTS_HEBREW_1 && id <= CONTROLID_BLUETOOTH_AUDIOVOX_CONTACTS_HEBREW_31)
	{
		...
		case MOUSE_STATUS_UP:
			{
				static wchar_t szMap[32] = {L'ק', L'ר', L'א', L'ט', L'ו', L'ן', L'ם', L'פ', L'ש', L'ד', L'ג', L'כ', L'ע', L'י', L'ח', L'ל', 
					L'ך', L'ף', L'ז', L'ס', L'ב', L'ה', L'נ', L'מ', L'צ', L'ת', L'ץ', L'?', L'?', L'?', L'׳', L'?'};
				r = szMap[i];	
			}
			break;
		case MOUSE_STATUS_TIMER:
...
void CPhoneBookPages::clickSearchResultPageGoBack(E_MOUSE_STATUS e)
{
	...
	case MOUSE_STATUS_UP:
		{
			/*if (m_ePhoneBookKeyBoardSearchLanguage == KEY_BOARD_LANGUAGE_ENGLISH)
				clickSearchPageEnglishKeyBoardEntrance(e);
			else if (m_ePhoneBookKeyBoardSearchLanguage == KEY_BOARD_LANGUAGE_ISRAEL)
				clickSearchPageIsRaelKeyBoardEntrance(e);*/
	...
	case MOUSE_STATUS_UP:
		{
			if (it != itTo)
			{
				...
				m_nSearchResultPageActiveIndex = i;
